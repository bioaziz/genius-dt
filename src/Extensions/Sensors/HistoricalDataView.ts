...
export function findNearestTimestampIndex(list: Date[], timestamp: Date, fractional: boolean = false): number {
    ...
    if (fractional && start < end) {
        return start + (timestamp.getTime() - list[start].getTime()) /
            (list[end].getTime() - list[start].getTime());
    } else {
        return (timestamp.getTime() - list[start].getTime() <
            list[end].getTime() - timestamp.getTime()) ? start : end;
    }
}
